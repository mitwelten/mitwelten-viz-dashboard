...
import {
  listAllPaxSensorData,
  listPaxSensorDataByNodeId,
} from '../../useCases/listPaxSensorData';
...
const findAll = (req, res) => {
  ...
  listAllPaxSensorData(accessor, limit, from, to)
    .then((data) => {
...
const findByNodeId = (req, res) => {
  ...
  if (!nodeId || !NODE_ID_REGEX.test(nodeId)) {
    ...
    listPaxSensorDataByNodeId(accessor, nodeId, limit, from, to)
      .then((data) => {
        res.status(200).json(data);
      })
      .catch((error) => res.status(500).json(error));
